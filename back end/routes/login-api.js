...
router.post("/login", async (req, res, next) => {
  await db();
    ...
    if(bcrypt.compare(user.password, getUser.password) && jwt.verify(getUser.token, secret)) {
        res.render("home", {
            getUser,
        })
    }
...
router.post("/update", async (req, res, next) => {
    await db();
      ...
      const getUser =await  userModel.findOne({email})
      console.log(getUser)
      if(bcrypt.compare(user.oldpassword, getUser.password) && jwt.verify(getUser.token, secret)) {
